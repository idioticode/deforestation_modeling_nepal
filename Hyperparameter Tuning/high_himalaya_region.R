...
# Perform grid search
for (mtry_val in c(3)) {
  for (nodesize_val in c(1)) {
    for (ntree_val in c(1500, 2000)) {
...
saveRDS(best_models, file = "D:/MISC/Deforestation_modeling_rasters/Final_Models/best_models_high_himalaya.rds")
...
high_himalaya_sf <- sf::st_read(
  "D:/MISC/points/Physiography/physiography_nepal_updated.shp"
) %>%
  dplyr::select(Physio)  %>% 
  dplyr::filter(Physio == "high_himalaya")


image <- ggplot()+
  geom_sf(
    data = high_himalaya_sf,
    color = "black",
    fill = "grey"
  )+
  geom_raster(
...
ggsave(
  'D:/MISC/Deforestation_modeling_rasters/Final_Images/SVG/high_himalaya_rf.svg',
  image,
  width = 12,
  height = 5
)
...
for (eta_val in c(0.5)) { # o.01 and 0.001 are removed for the second tuning. 
  for (depth_val in c(17)) {
    for (nrounds_val in c(500, 700, 1000)) {
