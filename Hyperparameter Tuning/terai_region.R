...
best_model_rf$nodesize <- best_params_rf$nodesize
...
ggsave(
  'D:/MISC/Deforestation_modeling_rasters/Final_Images/SVG/terai_rf.svg',
  image,
  width = 12,
  height = 5
)
...
xgb_data <- modeling_data


xgb_data$deforested <- as.numeric(as.character(xgb_data$deforested))
table(xgb_data$deforested)

# Assuming `modeling_data` is your data frame
xgb_result <- train_test_split(
  n_data = nrow(xgb_data[xgb_data$deforested == 1, ]),
  original_data = xgb_data,
  test_percentage = 30,
  dependent_variable = 'deforested',
  forest_column = 'lc_forest_2022'
)
...
# Perform grid search
for (eta_val in c(0.01, 0.1, 0.2, 0.3)) {
  for (depth_val in c(9, 11, 13)) {
    for (nrounds_val in c(300, 500, 700, 900)) {
...
best_model_xgb$eta <- 0.3
best_model_xgb$nrounds <- 900
best_model_xgb$max_depth <- 13
...
saveRDS(best_models, file = "D:/MISC/Deforestation_modeling_rasters/Final_Models/best_models_terai.rds")
