...
names(middle_mountain)

modeling_data <- middle_mountain %>% 
  dplyr::select(
...
saveRDS(best_models, file = "D:/MISC/Deforestation_modeling_rasters/Final_Models/best_models_middle_mountain.rds")
...
middle_mountain_sf <- sf::st_read(
  "D:/MISC/points/Physiography/physiography_nepal_updated.shp"
) %>%
  dplyr::select(Physio)  %>% 
  dplyr::filter(Physio == "middle_mountain")


image <- ggplot()+
  geom_sf(
    data = middle_mountain_sf,
    color = "black",
    fill = "grey"
  )+
  geom_raster(
...
ggsave(
  'D:/MISC/Deforestation_modeling_rasters/Final_Images/SVG/middle_mountain_rf.svg',
  image,
  width = 12,
  height = 5
)
...
# Perform grid search
for (eta_val in c(0.1, 0.2, 0.3, 0.4)) { # o.01 and 0.001 are removed for the second tuning. 
  for (depth_val in c(5, 7, 9, 11, 13, 15)) {
...
best_model_xgb$eta <- best_params_xgb$eta #0.4
best_model_xgb$nrounds <- best_params_xgb$nrounds # 900
